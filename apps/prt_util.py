...
def testPRT(dir_path, n=40):
    if dir_path[-1] == '/':
        dir_path = dir_path[:-1]
    for subject in sorted(os.listdir(dir_path)):
        print(subject)
        sub_path = os.path.join(dir_path, subject)
        sub_name = sub_path.split('/')[-1]
        obj_path = os.path.join(sub_path, sub_name + '.obj')
        
        os.makedirs(os.path.join(sub_path, 'bounce'), exist_ok=True)
        
        if os.path.exists(os.path.join(sub_path, 'bounce', 'face.npy')):
             print(subject, 'exits')
             os.remove(os.path.join(sub_path, 'bounce', 'face.npy'))
             os.remove(os.path.join(sub_path, 'bounce', 'bounce0.txt'))
             print('removed')
        
        PRT, F = computePRT(obj_path, n, 2)
        np.savetxt(os.path.join(sub_path, 'bounce', 'bounce0.txt'), PRT, fmt='%.8f')
        np.save(os.path.join(sub_path, 'bounce', 'face.npy'), F)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-i', '--input', type=str, default='/userhome/cs/yukang/data/THuman2.0/bad_Aligned_smpl')
