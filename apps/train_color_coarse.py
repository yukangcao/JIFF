...
def train_color(opt):
    ...
    train_dataset = TrainDataset_coarse(opt, phase='train')
    test_dataset = TrainDataset_coarse(opt, phase='test')
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        for train_idx, train_data in enumerate(train_data_loader):
            ...
            if len(opt.gpu_ids.split(",")) > 1:
                resC, error = netC_dp.forward(image_tensor, netG.get_im_feat(), color_sample_tensor, calib_tensor, labels=rgb_tensor)
            else:
        ...
        with torch.no_grad():
            ...
            if not opt.no_num_eval:
                ...
                test_color_error = calc_error_color_coarse(opt, netG, netC, cuda, test_dataset, 100)
                print('eval test | color error:', test_color_error)
                test_losses['test_color'] = test_color_error

                print('calc error (train) ...')
                train_dataset.is_train = False
                train_color_error = calc_error_color_coarse(opt, netG, netC, cuda, train_dataset, 100)
