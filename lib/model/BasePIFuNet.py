...
class BasePIFuNet(nn.Module):
    def __init__(self,
        ...
        self.error_term = error_term # MSE
    ...
    def filter_tdmm(self, face_tdmm):
        ...
        store all intermediate features.
        '''
#        self.im_tdmm_feat_list = self.tdmm_filter(face_tdmm)

        # If it is not in training, only produce the last im_feat
        # print(self.im_feat_list[-1].cpu().detach().numpy().shape)
#        if not self.training:
#            self.im_tdmm_feat_list = [self.im_tdmm_feat_list[-1]]
        None

    def query(self, points, calibs, region=None, transforms=None, labels=None, zrange=None):
        '''
